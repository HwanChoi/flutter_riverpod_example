...
export async function invokeGemini(options) {
    ...
    const timeoutPromise = new Promise((_, reject) => {
        timeoutHandle = setTimeout(() => {
            child.kill("SIGKILL");
            reject(new GeminiInvocationError(`Gemini invocation timed out after ${timeoutMs}ms`, -1, Buffer.concat(stdoutChunks).toString("utf8"), Buffer.concat(stderrChunks).toString("utf8")));
        }, timeoutMs);
    });
    let closeResult;
    try {
    ...
    if (exitCode !== 0) {
        throw new GeminiInvocationError(`Gemini exited with code ${exitCode}`, exitCode, stdout, stderr);
    }
